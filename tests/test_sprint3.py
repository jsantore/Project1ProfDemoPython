...
def test_get_excel():  # this is beyond the test I asked for
    ...
    sheet.append(['Company Name', "Posting Age", "Job Id", "Country", "Location", "Publication Date",
                  "Salary Max", "Salary Min", "Salary Type", "Job Title"])
    for row in range(10):
        sheet.append(
            [f"Company-{row}", f"Posted {row} days ago", f"Job-{row}", "US", f"Bridgewater, MA",
             1235324534215, f"{(row + row) * 10000}", f"{row * 10000}", f"yearly", "Software Developer Level {row}"]
        )
    excel_worbook.save("test.xlsx")
    test_data = DataProcessing.get_excel_data("test.xlsx")
    assert len(test_data) == 10  # we put ten peices of data plus the header - we should get that out.
